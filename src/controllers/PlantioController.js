...
class PlantioController {
    async buscaPlantios(req, res) {
        try {
            const require = await db.query("SELECT * FROM plantios ORDER BY plantio_id ASC");
      
            const plantios = require.rows;
  
            if (plantios.length === 0) return res.status(404).json("Nenhum plantio encontrado");
